...
def get_tracks_sample_by_genres(sample_size, genres):
    sql_engine = create_engine('mysql+pymysql://root:password@localhost/spotify_music_matcher', pool_recycle=3600)
    genres_query = 'SELECT DISTINCT genre FROM artist_n_genre'
    db_genres = pd.read_sql(genres_query, sql_engine)
    db_genres = db_genres['genre'].tolist()
    genres_str = ''
    for genre in genres:
    ...
    print(genres_str)
    tracks_query = 'select distinct track_name, artist_name, tracks.track_id, genre ' \
                   ...
                   'where genre in (' + genres_str + ');'

    tracks_df = pd.read_sql(tracks_query, sql_engine)
    return tracks_df.groupby('genre').apply(lambda s: s.sample(min(len(s), sample_size)))
