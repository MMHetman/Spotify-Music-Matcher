...
from keras.layers import Input, concatenate, Conv2D, Conv2DTranspose, Dropout, LeakyReLU, BatchNormalization, \
    AveragePooling2D, Flatten, Reshape, GaussianNoise
...
class ALIModel:
    ...
    def discriminator_model(self):
        ...
        Dx = compose_layers([
            ...
            Conv2D(filters=256, kernel_size=(4, 3), strides=2, padding=self.padding_type,
                   kernel_regularizer=l1_l2(*self.reg_params)),
            LeakyReLU(alpha=self.leaky_relu_slope),
            BatchNormalization(),
        ...
        conv2 = Conv2D(filters=1024, kernel_size=(1, 1), strides=1, padding=self.padding_type,
                       kernel_regularizer=l1_l2(*self.reg_params))
        conv3 = Conv2D(filters=1, kernel_size=(1, 1), strides=1, padding=self.padding_type,
                       kernel_regularizer=l1_l2(*self.reg_params))
